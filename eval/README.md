...
`demo_check` 默认离线；`--live` 执行最小 DeepSeek/MiMo 请求。真实消融复用缓存 transcript，不重新执行 ASR；原始 trace 位于被忽略的 `.mini-openclaw/`，汇总 JSON 跟踪在 `eval/planning_ablation_results.json`。
